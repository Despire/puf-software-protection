...
void Checksum::run(llvm::Module &M, const std::vector<llvm::Function *> &funcs) {
    auto &ctx = M.getContext();
    for (auto &func: funcs) {
        // TODO: correctly pick target functions. also the target functions should be in the table.
        patch_function(ctx, M, *func, funcs);
    }
}
...
Checksum::patch_function(
    ...
    auto split_block = add_checksum(ctx, M, &F, rng);
...
llvm::Function *Checksum::generate_checksum_func(llvm::Module &M) {
    static int i = 0;
    auto &ctx = M.getContext();
    llvm::Function *checksum_func = llvm::Function::Create(
            llvm::FunctionType::get(

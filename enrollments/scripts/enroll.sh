...
for num in 50; do # "num_of_measurements (used in the JSON config, i.e. this would equal to 1)
    ...
    for ((i=1; i<=$replication; i++)); do
        ...
        ssh -q root@beaglebone.local insmod /home/debian/puf/dram_puf.ko puf_start_address=${puf_start_address} puf_block_1_size=${puf_size}
        echo "recording dram cells at $sleep_seconds secs of decay"
        sleep ${sleep_seconds}
        ssh -q root@beaglebone.local rmmod dram_puf

        ssh -q root@beaglebone.local cat /dev/puf_start_address >  "${folder_name}/BBB_${iter}_${num}sec"
